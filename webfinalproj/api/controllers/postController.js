...
async function create(req, res) {
  try {
    const { email, postName , postimgUrl } = req.body;
    const post = await postService.createPost(email, postName,postimgUrl);
...
async function getallposts(req, res) {
    try {
      const { email, postName ,postimgUrl} = req.body;
      const post = await postService.getallposts(email, postName,postimgUrl);
